...
```{r}
votechoices <- c("pes21_votechoice2021", "cps21_votechoice")
...
selected_var <- unique(c(votechoices, mia_vars, extra_vars, check_disengaged, check_low_quality))
...
ces_feature <- ces_selected_converted %>% 
  select(all_of(c(votechoices, feature_vars)), disengaged_count, low_quality_count)
...
```{r}
ces_feature$votechoice <- ifelse(!is.na(ces_feature$pes21_votechoice2021), 
                                  as.character(ces_feature$pes21_votechoice2021), 
                                  as.character(ces_feature$cps21_votechoice))
...
#summary(ces_feature$votechoice)
```
